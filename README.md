...
- **V2.0** - 多位置版本：支持多个点击位置，可指定点击顺序，按顺序循环点击
  - **V2.1** - 最新优化版本：增强键盘快捷键、智能重复检测、自动保存优化、用户体验提升等
...
- **💾 自动保存优化**：
  - 表格编辑后自动保存（无需手动点击保存按钮）
  - 添加/删除位置后自动保存配置
  - 清空位置后自动保存
  
- **✨ 用户体验提升**：
  - 添加位置后自动选中新添加的行
  - 删除位置后智能保持选中状态（选中相邻位置）
  - 清空位置时显示位置数量，操作更明确
  - 验证失败时自动选中问题位置，便于快速定位
...
- **🛡️ 输入验证增强**：
  - 表格编辑时进行严格的输入验证（空值检查、数字格式验证）
  - 坐标范围自动验证（屏幕边界检查）
  - 更友好的错误提示（包含操作建议和位置信息）
...
4. **管理位置**：
   - **删除位置**：
     - 选中后点击"删除选中"按钮
     - 或按 `Delete` 键（表格聚焦时）快速删除
   - **测试位置**：
     - "测试选中位置"：单独测试选中的位置
     - "测试所有位置"：按顺序测试所有位置，验证整体流程
   - **清空所有**：点击"清空所有"可清空所有位置（显示数量并需确认）
